...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    // ===================== Command umum =====================
    if (text === '/start') {
      const pesan = `Selamat datang *${username || 'pengguna'}* 👋\n\n` +
        "Silakan pantau di sini untuk mengetahui kondisi logger.\n\nTerimakasih 🙏";
...
async function cekLoggerOffline() {
  try {
    const [lokasiList] = await db.query('SELECT idmet, nama FROM lokasi');
